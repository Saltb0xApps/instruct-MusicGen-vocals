...
from src.audiocraft.modules.rope import RotaryEmbedding
from src.audiocraft.modules.streaming import StreamingModule
...
class StreamingMultiheadAttention(StreamingModule):
    ...
    def __init__(self, embed_dim: int, num_heads: int, dropout: float = 0.0, bias: bool = True,
        ...
        if self.custom:
            ...
            # for lora
            self.q_proj = nn.Linear(out_dim // 3, out_dim // 3, bias=bias, **factory_kwargs)
            self.k_proj = nn.Linear(out_dim // 3, out_dim // 3, bias=bias, **factory_kwargs)
            self.v_proj = nn.Linear(out_dim // 3, out_dim // 3, bias=bias, **factory_kwargs)

            self.q_proj.weight.data = self.in_proj_weight.data[:(out_dim // 3)].float()
            self.k_proj.weight.data = self.in_proj_weight.data[(out_dim // 3): 2 * (out_dim // 3)].float()
            self.v_proj.weight.data = self.in_proj_weight.data[2 * (out_dim // 3):].float()
